...
def get_item_price_name(args):
	conditions = " WHERE item_code = '%s' AND price_list = '%s' AND min_qty <= '%s' AND uom = '%s'" \
				 % (args.item_code, args.price_list, args.qty, args.uom)
	if args.customer and not args.supplier:
		conditions += "AND customer= '%s'" % (args.customer)

	if args.supplier and not args.customer:
		conditions += "AND supplier= '%s'" % (args.supplier)
...
def get_item_price(args, item_code):
	conditions = "WHERE item_code = '%s' AND price_list = '%s' AND min_qty <= '%s' AND uom = '%s' AND ('%s' BETWEEN valid_from AND valid_upto)" \
				  % (item_code, args.price_list, args.qty, args.uom, args.transaction_date)
	...
	if frappe.db.get_value("Price List", args.price_list, "currency") == args.currency \
		...
		if frappe.has_permission("Item Price", "write"):
			...
			item_price = frappe.get_doc({
				...
				"valid_upto": add_days(args.transaction_date, 30)
			})
...
def get_price_list_rate_for(args, item_code):
	...
	if price_list_rate:
		...
		general_price_list_rate = frappe.db.sql("""
				...
				{'item_code': item_code, 'uom': args.uom, 'price_list': args.price_list})
